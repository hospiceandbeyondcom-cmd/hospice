...
export const metadata = {
  title: "Myths and Truths | Hospice and Beyond",
  description: "Clearing misconceptions, revealing the truth about compassion, comfort, and dignity in hospice care.",
  alternates: {
    canonical: "https://hospiceandbeyond.com/myth", // This fixes the error in your image
  },
};
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col">
      ...
      <section className="relative w-full h-[60vh] md:h-[80vh] flex items-center justify-center overflow-hidden bg-gray-900">
        ...
        <div className="relative z-10 text-center px-6 transition-all duration-1000 animate-in fade-in slide-in-from-bottom-10">
          <h1 className="text-white text-4xl md:text-6xl font-bold drop-shadow-lg">Myths and Truths About Hospice Care</h1>
          <p className="text-white text-lg md:text-xl mt-3 drop-shadow-md max-w-2xl mx-auto">Clearing misconceptions, revealing the truth about compassion, comfort, and dignity in hospice care.</p>
        </div>
      </section>

      <section className="max-w-6xl mx-auto w-full px-6 py-20">
        <h2 className="text-3xl md:text-4xl font-semibold text-center mb-12 text-[#006D66]">15 Common Myths and Their Truths</h2>
        <div className="grid md:grid-cols-2 gap-8">
          {myths.map((item, index) => (
            <div key={index} className="border border-gray-200 rounded-2xl p-6 bg-white shadow-sm hover:shadow-md transition-all hover:-translate-y-1">
              <h3 className="text-xl font-semibold mb-3 text-[#7D5F42]">Myth {index + 1}: {item.myth}</h3>
              <p className="text-gray-700 leading-relaxed"><span className="font-medium text-[#006D66]">Truth:</span> {item.truth}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 text-center" style={{ background: "linear-gradient(90deg, #006D66, #7D5F42)" }}>
        <h3 className="text-white text-2xl md:text-3xl font-semibold mb-4">Understanding Brings Peace.</h3>
        <p className="text-white mb-6 text-lg">Our team is always here to clarify, support, and guide with honesty and compassion.</p>
        <a href="/contact" className="inline-block px-8 py-3 bg-white text-[#006D66] font-semibold rounded-full shadow-md hover:scale-105 transition-transform">Contact Hospice and Beyond</a>
      </section>
      <Footer />
    </div>
  );
